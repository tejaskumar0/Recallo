...
export const fetchContentByUserAndFriend = async (userId: string, friendId: string): Promise<Content[]> => {
  try {
    const response = await fetch(`${API_URL}/quiz/content/user/${userId}/friend/${friendId}`);
    if (!response.ok) {
      throw new Error('Network response was not ok');
    }
    return await response.json();
  } catch (error) {
    console.error('Error fetching content for user-friend pair:', error);
    return [];
  }
};
